...
| `checkpoint.py` prints `SKIP: no changes` | Not a failure. The task legitimately produced no diff. Record it with `update_loop.py <feature> --task-done TN --no-diff`, or detection will re-dispatch it forever. |
